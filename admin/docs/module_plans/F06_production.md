...
- **不做**：SAM 遮罩繪製（sam_refine / sam_weighted）、合併色塊 / 消邊界 後處理工具（輪廓平滑已下架 2026-04-30，見 docs/requirements/admin_production.md §1.6）
- 呼叫客製 PATH (custom_request_id) 走得通，但建立 job 後不主動跳到 F05 報價頁

### F06-B（後續迭代）
- SAM 遮罩繪製工具（含 polygon 繪圖、sam_points 標記、re-build mask）
- 後處理工具（合併色塊 / 消邊界） — 需要 click-to-pick 互動 + canvas 操作
...
> smooth-contour 已下架（2026-04-30），見 docs/requirements/admin_production.md §1.6 註。
