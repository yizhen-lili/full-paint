...
new_user
  → 監聽：用戶 email 驗證完成事件
  → 條件：is_active = true
  → 動作：建立 user_coupon 記錄

spend_reward
  → 監聽：訂單狀態 → completed
  → 條件：order.total >= trigger_threshold AND is_active = true
  → 動作：建立 user_coupon 記錄

returning_loyal
  → 監聽：訂單狀態 → completed
  → 條件：order.total >= 1000
          AND 用戶有其他 completed 訂單
          AND is_active = true
  → 動作：建立 user_coupon 記錄

**同時觸發規則**

同一筆訂單若同時符合 spend_reward 與 returning_loyal 條件，只發放 returning_loyal 券一張（優先級較高），不重複發放 spend_reward。
邏輯順序：先判斷 returning_loyal → 若符合只發該券；否則再判斷 spend_reward。
...
public_code
  ...
  → 動作：建立 user_coupon 記錄，同時標記 is_used = true、used_at = now()（即時使用，不走先發放再使用的流程）
...
訂單狀態變為 `cancelled` 或 `refunded` 時，若該訂單有使用折扣券（`user_coupons.used_in_order_id` 對應此訂單），系統自動：
...
訂單退款（`refunded`）時，系統查找因此訂單觸發發放的回饋券（`spend_reward` / `returning_loyal`），條件：`user_coupons.source_order_id = 此訂單 id`（需在 user_coupons 新增 `source_order_id` 欄位記錄來源訂單）。

- 若該券**尚未使用**（`is_used = false`）→ 將 `expires_at` 設為當下時間，使其立即作廢
- 若該券**已使用**（`is_used = true`）→ 不處理（不追溯已完成的消費）
