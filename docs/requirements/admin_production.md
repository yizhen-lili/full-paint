...
| sam_weighted | 選取區佔色數比重較高，非選取區更簡化，需遮罩 | weight_ratio (0.5~0.8) |
...
- 系統根據圖片比例自動推薦 3 個標準規格供快速選用
- 可手動指定任意標準尺寸（單位：公分，格式 寬×高）
...
- `template.svg`、`snapped_rgb`：設為**私有**，禁止直接公開存取
- `filled_template.png`：設為**公開**（商品預覽圖需直接顯示於前台）
- 客製照片（`custom_requests.photo_url`）：設為**私有**，後端用簽名 URL 提供管理員存取（與 SVG 做法相同）
- 私有檔案的存取方式：後端用 Firebase Admin SDK（Service Account）產生**簽名 URL**，有效期限 15 分鐘。前端向後端請求臨時連結，後端驗證管理員 JWT 後回傳簽名 URL，前端用此 URL 直接從 Firebase 下載檔案。連結過期後自動失效，無法重複使用。
