...
- **候選池**：所有「已通過審核」的 production_job（不要求商品已上架）
  - 篩選條件：`ProductionJob.status='completed' AND ProductionJob.approved=True`
  - 包含兩條路徑：
    - 一般商品 job（綁定 ProductVariant → Product）— 即使尚未上架也可印（為了囤庫存）
    - 客製訂單 job（綁定 CustomRequest → User）— 客製訂單流程內的列印任務一併處理
  - label 顯示優先序：`Product.title` > `「客製 - {user.name}」` > `「製作任務 #{job_id前8碼}」`
  - 不去重、不排除（依使用者決議）
...
| GET candidates 列 approved=true 的 job（含一般商品 + 客製，不要求 on_sale）| 對應 | test_candidates_filter |
